...
class BootLoader:
    ...
    def Verify(self):
        continue_flag = QMessageBox.warning(self, "UNOS BootLoader", "Warning! UNOS is unstable in it's current state. Are you sure you want to continue?")


        if continue_flag == QMessageBox.Yes:
            QMessageBox.information(self, "UNOS BootLoader", "BootLoader: Launching UNOS")
            return True

        else:
            QMessageBox.information(self, "UNOS BootLoader", "BootLoader: Startup Failed")
            return False
            

    def StartupInterface(self):
